...
def test_three_rounds_full_flow(client, db_factory):
    ...
    r1 = _answer_round(client, session_id, 1, type="option", optionKey="c1")
    ...
    with db_factory() as db:
        ...
        assert answers[0].option_key == "c1"
...
def test_fake_generation_called_exactly_once(client):
    session_id = client.post("/api/sessions").json()["sessionId"]
    _answer_round(client, session_id, 1, type="option", optionKey="c1")
    _answer_round(client, session_id, 2, type="option", optionKey="c1")
...
def test_option_key_not_in_question_rejected(client):
    session_id = client.post("/api/sessions").json()["sessionId"]

    response = _answer_round(client, session_id, 1, type="option", optionKey="no_such_key")
...
def test_duplicate_submit_returns_stable_error(client, db_factory):
    ...
    first = _answer_round(client, session_id, 1, type="option", optionKey="c1")
    second = _answer_round(client, session_id, 1, type="option", optionKey="c1")
...
def test_concurrent_same_round_only_one_wins(client, db_factory):
    ...
    try:
        result_a = flow.submit_answer(
            session_a, session_id, 1, AnswerIn(type="option", option_key="c1")
        )
        session_a.commit()

        with pytest.raises(ApiError) as exc_info:
            flow.submit_answer(
                session_b, session_id, 1, AnswerIn(type="option", option_key="c2")
            )
            session_b.commit()
...
def test_cannot_skip_round_then_continue(client):
    ...
    wrong = _answer_round(client, session_id, 1, type="option", optionKey="c2")
    right = _answer_round(client, session_id, 2, type="option", optionKey="c1")
